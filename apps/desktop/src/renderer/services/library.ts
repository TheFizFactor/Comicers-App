...
import { Category, ReadingList } from '@/common/models/types';
...
const fetchReadingLists = (): ReadingList[] => {
  const val = persistantStore.read(`${storeKeys.LIBRARY.READING_LISTS}`);
  return val === null ? [] : JSON.parse(val);
};

const upsertReadingList = (readingList: ReadingList) => {
  const existingLists = fetchReadingLists().filter((list: ReadingList) => list.id !== readingList.id);
  persistantStore.write(
    `${storeKeys.LIBRARY.READING_LISTS}`,
    JSON.stringify([...existingLists, readingList]),
  );
};

const removeReadingList = (readingListId: string): void => {
  persistantStore.write(
    `${storeKeys.LIBRARY.READING_LISTS}`,
    JSON.stringify(fetchReadingLists().filter((list: ReadingList) => list.id !== readingListId)),
  );
};
